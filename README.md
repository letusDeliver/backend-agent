...
[![Tests](https://img.shields.io/badge/tests-255%20passing-33c481)](#testing)
...
- ✓ **Manual workspace cleanup** — a terminal, non-blocked real-mode task's isolated git worktree and task branch can be explicitly removed once you're done with it (`[ Cleanup Workspace ]` in Task Detail), freeing disk without ever touching your original repository or the durable artifact/attempt history; never automatic, never allowed while a conflict-blocked task still needs that exact workspace
- ✓ **Ground-truth diff content in review & Task Detail** — the specialist reviewer and the Task Detail UI both see the actual bounded `git diff` patch, not just a file list and line counts; an oversized diff is truncated explicitly (never silently) at a file boundary, and the reviewer is warned when it's looking at an incomplete patch
- ✓ **255 automated tests** (205 backend + 49 frontend) across the routing engine, repository inspector, reconciliation/conflict-detection logic, memory retrieval/candidate-lessons/approval, retry/attempt-archiving/startup-recovery, workspace cleanup/git-worktree lifecycle, ground-truth diff capture/truncation/review integration, API, SSE stream, git-worktree isolation, real-executor hardening (timeout/cancellation/ground-truth diff), and full end-to-end happy paths — plus a committed browser E2E test and CI running all of it on every push
...
```bash
npm test              # everything (254 tests: 205 backend + 49 frontend)
