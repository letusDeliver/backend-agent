...
After `implement()` finishes, whatever it actually changed is committed onto the task branch (`git add -A && git commit`, using a dedicated `Backend Engineering Agent` git identity — your global git config is never read or modified) and diffed against the base revision. That diff — file list, additions/deletions, summary, **and the actual patch text** (Phase 33) — is what the API and UI actually show you, computed from `git diff`, not taken from Claude's self-reported JSON. If Claude's implementation pass claims it changed files that don't actually appear in the diff, the diff wins.

### Diff content — bounded, never silently cut

The captured patch (`ExecutionDiff.patch`) is the same ground-truth `git diff` text described above, bounded to `MAX_DIFF_PATCH_CHARS` (default 200,000 characters, configurable). A patch that fits is stored and shown in full. A patch that doesn't is cut — preferring a whole-file boundary over a mid-hunk cut where the limit allows it — and `ExecutionDiff.truncated` is set `true`, with `totalPatchChars` always recording the full, untruncated length. Truncation is never silent: the stored patch itself carries an explicit `--- diff truncated ---` marker, the specialist reviewer's prompt is explicitly warned not to assume omitted sections are correct, and the Task Detail UI shows a "Diff truncated" notice with the shown/total character counts.

This same bounded patch is what the specialist reviewer actually reads — the review prompt embeds it directly, rather than asking the reviewer to separately re-derive it, framed explicitly as **untrusted repository content to inspect, not instructions to follow**: repository content (including a diff of it) is not a trusted input channel, so the prompt tells the reviewer to treat everything inside the patch — comments, strings, file content — as data, never as directives. This narrows (it does not eliminate) the platform's existing prompt-injection exposure for this one specific channel; it is not a general solution to untrusted repository content elsewhere in the pipeline. The patch is intentionally **not** repeated into SSE events or the on-disk event log — it already lives in `execution-report.json` (the canonical, API-served location), and duplicating a potentially large patch into every event would only bloat the event log for no reader that uses it.
...
The automated test suite (`npm test`, run in CI) never invokes the actual `claude` CLI — every real-mode test in `server/tests/real*.test.ts` and `server/tests/reviewDiff*.test.ts` uses a small fixture script standing in for it, so the suite needs no credentials, no network access to Anthropic's API, and runs deterministically. Those tests exercise 100% real git plumbing (a real throwaway repository, real `git worktree`/`diff`/`commit` commands) against the fixture CLI.

Validating against the **actual** `claude` CLI is a separate, manual step — documented with real output in `docs/PHASE_28_COMPLETION_REPORT.md`, and re-run for Phase 33 (documented in `docs/PHASE_33_COMPLETION_REPORT.md`) confirming the captured patch matches an independently-run `git diff` byte-for-byte — because it costs real API usage and takes real wall-clock time (each specialist call is a genuine Claude Code reasoning pass). To run it yourself:
...
- Diff content shown to the reviewer and in the UI (Phase 33) is bounded to `MAX_DIFF_PATCH_CHARS` (default 200,000 characters) — a genuinely huge implementation pass produces a truncated patch, not a longer one; the full change is always still available by checking out the task branch yourself.
- Framing the diff as untrusted content in the review prompt reduces, but does not eliminate, prompt-injection exposure — it only covers the diff-content channel. Other repository content the `claude` CLI reads agentically during analysis/implementation/review (source files, READMEs, comments) is not filtered or sanitized by this platform at all.
